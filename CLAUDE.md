...
error. Leave `VITE_BICING_API_BASE_URL` at its default there; same-origin is
what keeps the cookie, the passkey RP and the referrer-restricted Maps key
working. Staging shares the production config document, so changing a setting
there changes it for real.
...
That session is what the "My location" FAB drives (`geo.svelte.ts`): 30 minutes
long, one `getCurrentPosition` every 30 s, ended by leaving the Map screen or by
a refused permission. `maximumAge` must stay `0` — anything else lets the UA
answer every poll from one cached fix and the marker never moves. A single
interval drives both the refresh and the draining ring, and it derives elapsed
time from `Date.now()` rather than counting ticks because background tabs
throttle intervals to about one a minute.
...
The FAB is therefore a **three-state** control, not a toggle: idle starts a
session, following stops it, and panned-away resumes following rather than
ending the session — so one button both recentres and stops. The icon pulses
only while the map is actually following, which is what makes the middle state
legible. Ring, hub and icon are siblings inside the button on purpose: opacity
composites a subtree, so nesting the icon under the dimmed ring would cap how
bright it could pulse.
